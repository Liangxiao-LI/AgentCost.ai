"""Tool registry: schemas (Anthropic format), implementations, and metadata extraction.

All tool calls must go through call_tool() here — never called directly by the
agent. The observed_tool_call wrapper in executor.py is the only caller.
"""
...
# Format: { name, description, input_schema }  (not OpenAI's "parameters" key)
...
def _encoding(model: str) -> tiktoken.Encoding:
    # Anthropic doesn't publish a client-side tokenizer; cl100k_base gives a
    # reasonable approximation for schema token estimation.
    try:
...
def count_schema_tokens(schemas: list[dict], model: str) -> int:
    """Estimate tokens consumed by tool schemas in the context window."""
    return len(_encoding(model).encode(json.dumps(schemas)))
...
def extract_tool_metadata(tool_name: str, result: dict) -> dict:
    """Return source URL and traceability fields from a tool result."""
